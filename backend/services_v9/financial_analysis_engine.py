"""
Enhanced Financial Analysis Engine v3.2
Fixes: ROI, NPV, IRR calculations with realistic values
"""
...
class FinancialAnalysisEngineV32:
    """
    Correct financial calculations for LH new construction lease projects
    
    Key Fixes:
    1. Realistic ROI calculation
    2. Policy-based vs Private-based dual analysis
    3. Accurate 30-year cashflow modeling
    """
    ...
    def generate_complete_financial_analysis(self,
        ...
        """
        Complete financial analysis for Expert v3.2 report
...
if __name__ == "__main__":
    ...
    # Generate analysis
    result = engine.generate_complete_financial_analysis(cost_breakdown)
    
    # Print results
    print("=" * 80)
    print("FINANCIAL ANALYSIS RESULTS (v3.2)")
    print("=" * 80)
    print(f"\n총 투자비 (CAPEX): {result['cost_breakdown']['total_capex']}억원")
    print(f"LH 매입가: {result['lh_appraisal']['total_lh_purchase']}억원")
    print(f"사업 수익: {result['profitability']['profit']}억원")
    print(f"ROI: {result['profitability']['roi_percent']}%")
    print(f"IRR: {result['profitability']['irr_percent']}%")
    print(f"NPV: {result['npv']}억원")
    print(f"\n의사결정: {result['profitability']['decision']}")
    print(f"\n검증 결과:")
    ...
    for row in result['cashflow_table'][:5]:
        print(f"  Year {row['year']}: {row['description']:<20} | "
