...
class LHReportGeneratorV72Extended(LHReportGeneratorV72):
    ...
    def _safe(self, value, default="N/A"):
        """
        🔧 STABILITY PATCH: Safe value getter
    ...
    def _normalize_type_name(self, name: str) -> str:
        """
        🔧 STABILITY PATCH: Normalize type name for key matching
        Handles differences like "신혼·신생아" vs "신혼신생아"
        """
        if not name:
            return ""
    ...
    def _generate_extended_html_report(self, data: Dict[str, Any]) -> str:
        ...
        # Try multiple possible locations for type_demand_scores
        type_demand_scores = (
            data.get('type_demand_scores', {}) or 
            data.get('type_demand_v3_1', {}).get('type_scores', {}) or
            data.get('all_types_scores', {}) or
            {}
        )
        
        logger.info(f"📊 TypeDemand scores found: {type_demand_scores}")
        
        # Extract GeoOptimizer alternatives
        geo_alternatives = (
            data.get('geo_optimization', {}).get('recommended_sites', []) or
            data.get('geo_optimizer_v3_1', {}).get('alternatives', []) or
            []
        )
        
        logger.info(f"📊 GeoOptimizer alternatives found: {len(geo_alternatives)} items")
...
"""
    ...
    def _generate_type_demand_extended_section_fixed(
        ...
        # 🔧 PATCH: Normalize keys for matching
        normalized_scores = {}
        for key, value in type_demand_scores.items():
            normalized_key = self._normalize_type_name(key)
            normalized_scores[normalized_key] = value
        
        # 5개 타입 순회 (original names for display)
        type_mapping = {
            '청년': '청년',
            '신혼·신생아 I': '신혼·신생아 I',
            '신혼·신생아 II': '신혼·신생아 II',
        ...
        for display_name, original_name in type_mapping.items():
            # Try normalized key matching
            norm_name = self._normalize_type_name(original_name)
            score = normalized_scores.get(norm_name, 0)
            
            # Fallback to exact match
            if score == 0 and original_name in type_demand_scores:
                score = type_demand_scores.get(original_name, 0)
            
            # Apply safe getter
            score = self._safe(score, 0)
            ...
            # 현재 선택된 타입 강조
            norm_unit = self._normalize_type_name(unit_type)
            is_current = (norm_unit == norm_name)
            row_style = 'background: #e3f2fd; font-weight: bold;' if is_current else ''
            
            five_type_table += f"""
            <tr style="{row_style}">
                <td><strong>{display_name}</strong> {'👈 선택' if is_current else ''}</td>
...
"""
    ...
    def _generate_zone_full_data_table(self, zone_data: Dict) -> str:
        """
        🔧 PATCH: Generate full zoning data table (23 fields) with safe handling
        """
        fields = [
            ('zone_type', '용도지역'),
            ...
            ('development_restriction', '개발제한구역'),
            ('district_unit_plan', '지구단위계획'),
            ('urban_planning_zone', '도시계획구역'),
            ('green_belt', '녹지지역'),
            ('landscape_zone', '경관지구'),
            ('fire_district', '방화지구'),
            ('parking_zone', '주차장정비지구'),
            ('exclusive_zone', '전용지역'),
            ('general_zone', '일반지역'),
            ('residential_zone', '주거지역'),
            ('commercial_zone', '상업지역'),
            ('industrial_zone', '공업지역'),
            ('natural_zone', '자연환경보전지역'),
        ]
        
        table = "<table class='full-data-table'><tr><th>필드명</th><th>값</th><th>설명</th></tr>"
        for field_key, field_name in fields:
            # 🔧 PATCH: Safe getter with API error indication
            raw_value = zone_data.get(field_key)
            value = self._safe(raw_value)
            
            # Add API error indication if None
            if raw_value is None:
                value = f"{value} (API 조회 실패)"
...
"""
    ...
    def _generate_geo_optimizer_extended_section_fixed(
        ...
        # 🔧 PATCH: Ensure 3 alternatives (fill with placeholders if needed)
        safe_alternatives = list(alternatives) if alternatives else []
        while len(safe_alternatives) < 3:
            safe_alternatives.append({
                'site_id': f'ALT_PLACEHOLDER_{len(safe_alternatives)+1}',
                'address': 'N/A (대안 정보 없음)',
                'overall_score': 0,
                'strengths': ['정보 없음'],
                'recommendation_reason': '추가 분석 필요'
            })
        ...
        # 상위 3개 대안 표시 (safe getter 적용)
        for idx, alt in enumerate(safe_alternatives[:3], 1):
            site_id = self._safe(alt.get('site_id'), f'ALT_{idx:02d}')
            address = self._safe(alt.get('address'), 'N/A')
            overall_score = self._safe(alt.get('overall_score'), 0)
            ...
            strengths_list = alt.get('strengths', ['정보 없음'])
            if not strengths_list or len(strengths_list) == 0:
                strengths_list = ['정보 없음']
            strengths = ', '.join(str(s) for s in strengths_list[:2])
            
            alternatives_table += f"""
            <tr>
                <td>후보 {idx}</td>
                <td>{address[:30]}{'...' if len(address) > 30 else ''}</td>
...
"""
    ...
    def _generate_appendix_raw_data(self, data: Dict) -> str:
        """
        🔧 STABILITY PATCH: Generate FULL Raw JSON Appendix with safe handling
        
        변경사항:
        - 기존: 10,000자 제한 → 신규: 100,000자 제한
        - 모든 엔진 데이터 100% 출력
