...
<h4>💰 감정평가 영향</h4>
<p>감정평가는 '최근 3개월 거래사례'를 기준으로 하므로, 현재의 저평가 수준이 
매입가 산정에 반영될 가능성이 높다. 이는 사업자 입장에서 <strong>토지비 절감 효과</strong>를 
가져올 수 있으며, 재무 타당성 개선에 긍정적 요인으로 작용한다.</p>
</div>
"""
...
<h4>📊 시장 평가: 적정 가격</h4>
<p>현재 시장 가격은 적정 가치 범위 내에 있다. 이는 감정평가 시 
거래사례 기반의 객관적 평가가 가능함을 의미하며, LH 매입가 산정에서 
예측 가능성이 높다는 장점이 있다.</p>
</div>

<div class="highlight-box">
<h4>💰 감정평가 영향</h4>
<p>적정 시장 가격 수준에서는 감정평가액과 실제 거래가의 괴리가 적어, 
사업자의 토지 매입 협상 시 명확한 기준가를 제시할 수 있다.</p>
</div>
"""
        else:  # OVERVALUED
            narrative += f"""
...
<h4>⚠️ 시장 평가: 고평가</h4>
<p>현재 시장 가격은 적정 가치 대비 <strong>{self.fmt_pct(delta_pct, 1)}</strong> 높은 수준이다. 
이는 감정평가 과정에서 <strong>'하향 조정'</strong> 가능성을 시사하며, 
사업자가 토지를 시장가로 매입할 경우 감정평가액이 매입가를 하회할 리스크가 있다.</p>
</div>

<div class="strategy-box">
<h4>🎯 리스크 대응 전략</h4>
<ol>
<li>토지 매입 시 감정평가 결과 기준 조건부 계약 체결</li>
<li>LH 사전협의를 통한 감정평가 방향성 확인</li>
<li>공사비 연동형 감정평가 적용 가능성 검토</li>
</ol>
</div>
"""
...
<div class="analysis-box">
<h4>📊 재무 타당성 분석</h4>
<p>NPV가 음수({self.fmt(npv, 1)}억원)라는 것은 <strong>민간 PF 구조로는 수익성 확보가 어렵다</strong>는 
의미이다. 이는 다음 두 가지 요인에 기인한다:</p>
<ol>
<li><strong>LH 정책형 임대료 수준</strong>: 시세의 85% 수준으로 책정되어 민간 임대 대비 수익성 낮음</li>
<li><strong>높은 초기 투자비</strong>: 토지비 + 공사비 부담이 크며, 회수 기간이 장기화됨</li>
</ol>
</div>

<div class="highlight-box">
<h4>💡 정책적 타당성 관점</h4>
<p>다만 본 사업은 <strong>'LH 공공주택 공급'</strong>이라는 정책 목표를 기준으로 평가되어야 한다. 
LH는 수익성보다 <strong>'주거 복지 실현'</strong>을 우선하므로, NPV 음수는 사업 불가 판단의 
절대 기준이 아니다.</p>
</div>

<div class="strategy-box">
<h4>🎯 사업화 전략</h4>
<p>다음 전략을 통해 재무 구조 개선이 가능하다:</p>
<ol>
<li><strong>LH 직매입 방식</strong>: 사업자는 건설만 수행, 토지비 부담 제거</li>
<li><strong>공사비 연동형 감정평가</strong>: 공사비 기준 매입가 산정으로 수익성 확보</li>
<li><strong>정책자금 활용</strong>: LH 제공 저금리 자금(연 2.87%) 활용</li>
<li><strong>사업 규모 확대</strong>: 토지 면적 증가를 통한 규모의 경제 실현</li>
</ol>
</div>
"""
        else:
...
"""
        ...
        if recommendation == 'GO':
            narrative += f"""
<div class="highlight-box">
<h4>✅ 최종 판정: 즉시 실행 가능 (GO)</h4>
<p>본 사업은 <strong>즉시 실행 가능(GO)</strong> 수준이다. 모든 핵심 지표가 양호하며, 
LH 평가 통과 가능성이 매우 높다. 토지 확보 및 인허가 절차를 조속히 진행할 것을 권고한다.</p>
</div>
"""
        elif recommendation == 'CONDITIONAL':
            narrative += f"""
<div class="strategy-box">
<h4>⚠️ 최종 판정: 조건부 실행 가능 (CONDITIONAL GO)</h4>
<p>본 사업은 <strong>조건부 실행 가능(CONDITIONAL GO)</strong> 수준이다. 
기본적으로 사업성은 확보되었으나, 다음 조건이 충족되어야 한다:</p>
<ol>
<li>LH 사전협의를 통한 매입 조건 명확화</li>
<li>감정평가 방향성 확인 (공사비 연동형 적용 여부)</li>
<li>재무 구조 최적화 (정책자금 활용, 공사비 절감 등)</li>
</ol>
<p>위 조건 충족 시 사업 실행을 권고한다.</p>
</div>
"""
        elif recommendation == 'REVISE':
            narrative += f"""
<div class="warning-box">
<h4>🔄 최종 판정: 재검토 필요 (REVISE)</h4>
<p>본 사업은 <strong>재검토 필요(REVISE)</strong> 수준이다. 
현재 구조로는 재무 타당성이 부족하거나 시장 조건이 불리하므로, 
다음 개선 방안을 검토한 후 재평가할 것을 권고한다:</p>
<ol>
<li>사업 규모 확대 (인접 필지 추가 확보)</li>
<li>공사비 절감 방안 (VE, 자재 선정 최적화)</li>
<li>LH 특별 지원 프로그램 활용 검토</li>
<li>대체 입지 검토</li>
</ol>
</div>
"""
            ...
            narrative += f"""
<div class="warning-box">
<h4>❌ 최종 판정: 실행 불가 (NO-GO)</h4>
<p>본 사업은 <strong>실행 불가(NO-GO)</strong> 수준이다. 
